    ...
    def train(self, train_dataset, eval_dataset=None, batch_size=16, 
        ...
        for epoch in range(epochs):
            ...
            for batch in progress_bar:
                input_ids = batch.to(self.device, non_blocking=True)  # ← non_blocking for pinned memory
                ...
                if self.device == 'cuda':
                    ...
                    # Scale and step with GradScaler
...
def prepare_dataset(config, tokenizer=None):
    ...
    if config['training']['dataset'] == "tiny_shakespeare":
        ...
        dataset = load_dataset(config['training']['dataset'])["train"]
        texts = dataset["text"]
        ...
        if tokenizer is None:
            tokenizer = BPETokenizer(text=sample_text, special_tokens=["[PAD]", "[EOS]", "[UNK]"])
